...
pub unsafe fn cxxvec_to_vector3(cxx_vec: &UniquePtr<CxxVector<f64>>) -> Option<Vector3<f64>> {
    if cxx_vec.len() == 3 {
...
pub unsafe fn cxxvec_to_matrix3(cxx_vec: &UniquePtr<CxxVector<f64>>) -> Option<Matrix3<f64>> {
    if cxx_vec.len() == 9 {
